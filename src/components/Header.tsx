  ...
  return (
    <header className="sticky top-0 z-50 border-b border-grc-line bg-white/95 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
        ...
        <nav className="hidden items-center gap-6 text-sm text-grc-ink-muted md:flex">
          {nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="transition hover:text-grc-ink"
            >
              {item.label}
            </a>
          ))}
        ...
        <div className="flex items-center gap-3">
          <a
            ...
            className="hidden text-sm font-semibold text-grc-ink sm:block"
          >
            {brand.phone}
          </a>
          <a
