...
export function Directions() {
  return (
    <section
      id="directions"
      className="scroll-mt-20 border-b border-grc-line bg-grc-paper py-20"
    >
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-2">
          {directions.map((dir) => (
            <article
              key={dir.title}
              className="group overflow-hidden rounded-sm border border-grc-line bg-white shadow-sm transition hover:border-grc-orange/40 hover:shadow-md"
            >
              <div className="relative aspect-[16/10] overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-grc-ink/80 via-grc-ink/10 to-transparent" />
              </div>
              <div className="p-6 sm:p-8">
                <h3 className="font-[family-name:var(--font-display)] text-2xl font-bold uppercase text-grc-ink">
                  {dir.title}
                </h3>
                ...
                <ul className="mt-4 flex flex-wrap gap-2">
                  {dir.points.map((point) => (
                    <li
                      key={point}
                      className="rounded-sm border border-grc-line bg-grc-soft px-3 py-1 text-xs text-grc-ink-muted"
                    >
                      {point}
                    </li>
                  ))}
